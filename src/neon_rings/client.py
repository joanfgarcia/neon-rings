...
class RingsClient:
    def __init__(self, url: str = "", transport: Optional[RingsTransport] = None):
        if not transport:
            if not url:
    ...
    async def send_message(self, target_id: str, payload: Any):
        ...
        return await self._transport.call("send_message", {
            "target_id": target_id,
            "payload": payload
        })
    ...
    async def dht_put(self, keypair: KeyPair, value: str):
        ...
        return await self._transport.call("dht_put", {
            "key": pub_hex,
            "value": value,
            "signature": signature
        })
