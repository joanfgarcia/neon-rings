...
class RingsServer:
	...
	async def process_rpc(self, req: dict[str, Any], websocket: websockets.ServerConnection) -> dict[str, Any] | None:
	...
	async def handle_send_message(self, params, websocket):
		...
		if target_ws:
			...
			notification = {"jsonrpc": "2.0", "method": "message_receive", "params": {"sender_id": sender_id, "payload": payload, "ts": time.time()}}
	...
	async def handle_dht_put(self, params, websocket):
		...
		try:
			...
			raise ValueError("Invalid signature format") from None
